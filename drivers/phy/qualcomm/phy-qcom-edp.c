...
/* Nord (NoardAU) eDP_DP2 PHY N4 HPG rev D §2.7.3: corrected LDO config register */
...
static int qcom_edp_phy_init(struct phy *phy)
{
	...
	/*
	 * AUX_CFG3 is documented as "programmed twice" (HPG Table 2-1-a):
	 * the init array above carries the first write (0x01 on nord), and
	 * this explicit override applies the second write (0x00). Harmless
	 * no-op for every other compatible, whose aux_cfg[3] is already 0.
	 */
...
/*
 * Nord (codename nordschleife) eDP_DP2 PHY, per "eDP1.5/DP2.0 PHY N4 HPG"
 * rev S. Register generation v8, reused wholesale (no new offset header) -
 * every value below traces to nord's own HPG, not borrowed from glymur.
 *
 * NOTE: the cfg struct and its per-generation tables are intentionally
 * named "noard", reproducing the HPG's own typo ("NoardAU") rather than
 * "nord" - this is the identifier the in-vehicle silicon validation used,
 * see nordschleife_dp_phy_description.md §8.1. Do not rename.
 */
...
/*
 * com_clk_fwd_cfg is intentionally not implemented for nord: the HPG does
 * not describe a CLK_FWD_CONFIG_1 sequence for this PHY, and the
 * qcom_edp_phy_ops_noard table below leaves this member NULL - see the
 * NULL check added to qcom_edp_phy_init().
 */
...
static int qcom_edp_com_configure_pll_noard(const struct qcom_edp *edp)
{
	...
	writel(0x00, edp->pll + DP_QSERDES_V8_COM_VCO_TUNE_CTRL);
	/* HPG note: "May be 0x17" - driver uses 0x1f */
	writel(0x1f, edp->pll + DP_QSERDES_V8_COM_BIAS_EN_CLKBUFLR_EN);
...
static int qcom_edp_ldo_config_noard(const struct qcom_edp *edp)
{
	...
	/*
	 * HPG rev D corrects the LDO config description (IPCAT/SWI is wrong
	 * here, see nordschleife_dp_phy_description.md §6.7/§8.3): DP mode
	 * disables the LDO entirely, eDP mode enables it with a corrected
	 * DP_PHY_LDO_CFG value.
	 */
	if (!edp->is_edp) {
		ldo_config = 0x00;
		phy_ldo_cfg = 0x00;
...
/*
 * Cfg struct name intentionally reproduces the HPG's own "NoardAU" typo
 * (noard, not nord) - see nordschleife_dp_phy_description.md §8.1. This
 * matches the identifier the in-vehicle silicon validation used; do not
 * rename to nord_edp_phy_cfg without a corresponding rebuild-and-retest.
 */
